                    ...
                    try:
                        ...
                        try:
                            if request_need_advert:
                                try:
                                    advert = Advert.objects.select_related('buttons_constructor').get(buttons_constructor__pk=old_btn_const.pk)

                                    advert.buttons_constructor = buttons_constructor
                                except:
                                    advert = Advert.objects.create(buttons_constructor=buttons_constructor, cabinet_webmaster=cabinet_webmaster)
                        ...
                        for w in wm_area_list:
                            if w.buttons_constructor.pk not in old_btns_pk_list:
                                try:
                                    if request_need_advert:
                                        try:
                                            advert = Advert.objects.select_related('buttons_constructor').get(buttons_constructor__pk=w.buttons_constructor.pk)
