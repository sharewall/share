    ...
    if wma_object:
        ...
        if advert:
            ...
            if advert.ad_type == 'BUY':
                answer += '$("div#sharewallContainer").append(\'<span id="sAdvBtn" style="width:41px; height:41px; display:inline-block; margin-right:15px; background-image: url(http://sharewall.ru/static/sharewall-template/'+btncr.btns_images.path.replace('_ci_', '_sq_') if btncr.form_buttons=='SQ' else btncr.btns_images.path+'); background-repeat: no-repeat; '+advert.btn_image.bg_position+' cursor: pointer; margin-left:-2px;"></span>\''
                if btncr.location_buttons == "VE":
                    answer += '+\'<br/>\''
                answer += ');'
        ...
        for sn in btncr.social_networks.split(','):
            ...
            answer += '   +\'<a data-share-sn="{0}" href=\'+new_href+\' style="width:41px; height:41px; display:inline-block; margin-right:15px; background-image: url(http://sharewall.ru/static/sharewall-template/{1}); background-repeat: no-repeat; {2}"></a>\''.format(sn, btncr.btns_images.path.replace('_ci_', '_sq_') if btncr.form_buttons=='SQ' else btncr.btns_images.path, list_sn_unic[sn].img_bd_pos)#
            if btncr.location_buttons == "VE":
                answer += '+\'<br>\''
            answer += ');'

        answer += '$("div#sharewallContainer").append(\'<a target="_blank" href="//sharewall.ru" style="width:41px; height:41px; display:inline-block; margin-right:15px; background-image: url(http://sharewall.ru/static/sharewall-template/'+btncr.btns_images.path.replace('_ci_', '_sq_') if btncr.form_buttons=='SQ' else btncr.btns_images.path+'); background-repeat: no-repeat; background-position: -328px 0px;"></a>\');'
