    ...
    for area in areas:
        ...
        for a in area_per_day_list:
            ...
            if temp_areaPDay_date in dates:
                ...
                if for_1:
                    try:
                        temp_index = next(index for (index, d) in enumerate(temp_area_per_day_statistic) if d['pk'] == a.webmaster_area.pk)

                        if temp_today_share_counter > temp_area_per_day_statistic[temp_index]['share']:
                            temp_area_per_day_statistic[temp_index]['share'] = temp_today_share_counter

                        if temp_today_social_counter > temp_area_per_day_statistic[temp_index]['social']:
        ...
        if not for_1:
            ...
            for share, social in [[t['share'], t['social']] for t in temp_area_per_day_statistic]:
                statistic['shares'] += int(share)
                statistic['socials'] += int(social)
